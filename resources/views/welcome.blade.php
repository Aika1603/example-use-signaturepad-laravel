...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <link rel="stylesheet" href="{{ asset('assets/signature-pad.css') }}">
    ...
    <body class="bg-light" style="margin-top:50px;">
        <div class="bg-white container shadow p-3 mb-5 bg-white rounded">
            ...
            <form action="{{ route("store") }}" method="post">
                <div class="mt-4 row">
                    ...
                    <div class="col-sm-6 col-lg-4">
                        <div class="card">
                            <div class="signature-img bg-light" id="img_petugas_signature">
                                @if (@$item->petugas_signature != '')
                                    <img src="{{ Illuminate\Support\Facades\Storage::disk('public')->url("signature/$item->petugas_signature") }}" class="card-img-top">
                                @endif
                            </div>
                            <div class="card-body">
                                <h5 class="card-title">TTD Petugas</h5>
                                <textarea name="petugas_signature" id="petugas_signature" style="display:none"></textarea>
                                <button class="btn btn-primary" type="button" onclick="createSignature('petugas_signature')">Buat Tanda Tangan</button>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-6 col-lg-4">
                        <div class="card">
                            <div class="signature-img bg-light" id="img_customer_signature">
                                @if (@$item->customer_signature != '')
                                    <img src="{{ Illuminate\Support\Facades\Storage::disk('public')->url("signature/$item->customer_signature") }}" class="card-img-top">
                                @endif
                            </div>
                            <div class="card-body">
                                <h5 class="card-title">TTD Customer</h5>
                                <textarea name="customer_signature" id="customer_signature" style="display:none"></textarea>
                                <button class="btn btn-primary" type="button" onclick="createSignature('customer_signature')">Buat Tanda Tangan</button>
                            </div>
                        </div>
                    </div>
        ...
        <div class="modal fade" id="Signature" tabindex="-1" aria-labelledby="modal" aria-hidden="true">
            <div class="modal-dialog modal-md">
                <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modal">New Signature</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <input type="hidden" id="signature-target" value="">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="SaveSignature()">Save </button>
                </div>
                </div>
            </div>
            </div>
        ...
        <script>
            $('#Signature').on('shown.bs.modal',function(e){
            ...
            const createSignature = (target) => {
                $('#signature-target').val(target);
                $('#Signature').modal('show');
            }

            const SaveSignature = () => {
                let target = $('#signature-target').val();
                if (signaturePad.isEmpty()) {
                    alert("Please provide a signature first.");
                } else {
                    let dataURL = signaturePad.toDataURL();
                    $('#img_'+target).html(`<img src="${dataURL}" class="card-img-top">`);
                    $('#'+target).val(dataURL);
